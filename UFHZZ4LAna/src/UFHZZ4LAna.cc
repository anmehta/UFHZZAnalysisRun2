...
private:
  ...
  vector<double> Z_noFSR_pt; vector<double> Z_noFSR_eta; vector<double> Z_noFSR_phi;
...
void UFHZZ4LAna::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  //std::cout << "[INFO] use 2l2q code" << endl;
  ...
  if (isMC && reweightForPU) {
    ...
    if(verbose) std::cout << "N true interations = " <<npv<<endl;
    nInt = npv;
...
//===================================================================================================
  if(theVertex >= 0 && (isMC || (!isMC && trigConditionData)) ){
    ...
    for(unsigned int i = 0; i < lep_ptreco.size(); i++){
      ...
      if (abs(lep_ptid[i])==11){
        ...
        lep_missingHits.push_back(recoElectrons[lep_ptindex[i]].gsfTrack()->hitPattern().numberOfAllHits(reco::HitPattern::MISSING_INNER_HITS));
        double perr = 0.0;
    ...
    if(verbose) std::cout << "adding taus to sorted list" <<endl;
  ...
  if(ntight>=(uint)skimTightLeptons){
    ...
    for(unsigned int i=0; i<jets->size(); ++i){
      ...
      if(verbose) std::cout << "jet pu mva " <<jpumva<<endl;
      /*
      if(jet.pt()>20){
          if(abs(jet.eta())>3.){
      ...
      */
      ...
      if( jetHelper.patjetID(jet,year)>=jetIDLevel ){
        ...
        if(fabs(jet.eta()) < jeteta_cut){ ///move all pt cut after JES/JER corrections
          ...
          if(verbose) std::cout << "adding pu jet scale factors " <<endl;
          bool dropit=false;
          if(abs(jet.eta())>3.0 && isMC){
            TRandom3 rand;
            rand.SetSeed(abs(static_cast<int>(sin(jet.phi())*100000)));
            float coin = rand.Uniform(1.0);
    ...
    if(isCode4l && foundHiggsCandidate){
      ...
      if(verbose) std::cout << nZXCRFailedLeptons<<" failing leptons in higgs candidate"<<endl;
    ...
    if (isCode4l && doMela && foundHiggsCandidate){
      ...
      if(njets_pt30_eta4p7>=2){
        ...
        if(jetqgl0<0.0){ // if the q/g tagger has the error value (-1.), take a random one instead
          TRandom3 rand;
          rand.SetSeed(abs(static_cast<int>(sin(jet_phi[jet1index])*100000)));
          jetqgl0 = rand.Uniform();
        }
        if(jetqgl1<0.0){ // if the q/g tagger has the error value (-1.), take a random one instead
          TRandom3 rand;
                                   ...
                                   std::vector<pat::Jet> selectedMergedJets)
{
  ...
  for(unsigned int k = 0; k<goodJets.size(); k++){
    ...
    for(unsigned int i=0; i<Nleptons; i++){
      ...
      bool cand_lep=false;
      for(int l=0; l<4; l++){
        if((int)i==lep_Hindex[i]) cand_lep=true;
      }
